...
private extension Notification.Name {
    static let remoteFlowShouldShowFallback = Notification.Name("remoteFlowShouldShowFallback")
}

private enum FlowFallback {
...
private final class FlowBootstrapViewModel: ObservableObject {
    ...
    @MainActor
    func handleFlowFailure(reason: String) {
    ...
    private func bootstrap(trigger: String) async {
        ...
        if let cachedTaskLink = normalizeURLString(UserDefaults.standard.string(forKey: "taskLink")),
           trigger == "start" {
            await MainActor.run { self.phase = .remote(cachedTaskLink) }
        }
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(FlowResponse.self, from: data)
            ...
            if let taskLink = normalizeURLString(decoded.response) {
                ...
                await MainActor.run { self.phase = .remote(taskLink) }
            } else {
                await MainActor.run { self.phase = .native }
            }
        } catch {
    ...
    private func resolvedClientUUID() -> String {
        let fromWorking = UserDefaults.standard.string(forKey: "workingClientUUID") ?? ""
        if !fromWorking.isEmpty { return fromWorking }
...
struct RootContentView: View {
    @StateObject private var flowBootstrapViewModel = FlowBootstrapViewModel()
    ...
    private let flowFailedPublisher = NotificationCenter.default.publisher(
        for: .remoteFlowShouldShowFallback
    )

    var body: some View {
        Group {
            switch flowBootstrapViewModel.phase {
            case .loading:
                ZStack {
            ...
            case .native:
                ContentView()
            case .remote(let taskLink):
                RemoteScreen(taskLink: taskLink)
                    .ignoresSafeArea()
            }
        }
        .onAppear {
            ...
            flowBootstrapViewModel.start()
        }
        .onReceive(tokenReceivedPublisher) { _ in
            flowBootstrapViewModel.handlePushOrTokenUpdate(trigger: "tokenReceived")
        }
        .onReceive(flowFailedPublisher) { note in
            let reason = note.userInfo?[FlowFallback.reasonKey] as? String ?? "unknown"
            flowBootstrapViewModel.handleFlowFailure(reason: reason)
        }
    }
}

private struct ServiceBootstrapLoadingView: View {
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [
            ...
            VStack(spacing: 18) {
                ...
                Text("Please wait while we securely prepare your session.")
...
private struct RemoteScreen: UIViewControllerRepresentable {
    let taskLink: String

    func makeUIViewController(context: Context) -> RemoteViewController {
        let controller = RemoteViewController()
        controller.taskLink = taskLink
        return controller
    }

    func updateUIViewController(_ uiViewController: RemoteViewController, context: Context) {
        uiViewController.taskLink = taskLink
        uiViewController.loadIfNeeded()
    }
}

private final class RemoteViewController: UIViewController, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    var taskLink: String = ""

    private var primaryView: WKWebView?
    ...
    func loadIfNeeded() {
        guard primaryView != nil else { return }
    ...
    private func configureMainView() {
        ...
        viewConfiguration.userContentController.addUserScript(interceptorScript)
        viewConfiguration.userContentController.removeScriptMessageHandler(forName: "lmLogger")
        viewConfiguration.userContentController.add(self, name: "lmLogger")

        let createdPrimaryView = WKWebView(frame: .zero, configuration: viewConfiguration)
        createdPrimaryView.customUserAgent = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1"
        createdPrimaryView.navigationDelegate = self
        createdPrimaryView.uiDelegate = self
        createdPrimaryView.allowsBackForwardNavigationGestures = true
        createdPrimaryView.translatesAutoresizingMaskIntoConstraints = false

        view.addSubview(createdPrimaryView)
        NSLayoutConstraint.activate([
            createdPrimaryView.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor),
            createdPrimaryView.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor),
            createdPrimaryView.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor),
            createdPrimaryView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor)
        ])

        primaryView = createdPrimaryView
    }
    ...
    private func loadMainURL(_ url: URL, reason: String, resetRetries: Bool) {
        ...
        lastMainURL = url
        primaryView.load(URLRequest(url: url))
    }
    ...
    func webView(_ browserView: WKWebView, didFinish navigation: WKNavigation!) {
        if browserView === self.primaryView {
            mainLoadRetryCount = 0
    ...
    func webView(_ browserView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        handleNavigationError(error, for: browserView, stage: "didFail")
    }

    func webView(_ browserView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        handleNavigationError(error, for: browserView, stage: "didFailProvisional")
    }

    func webViewWebContentProcessDidTerminate(_ browserView: WKWebView) {
        if browserView !== self.primaryView {
            closePopupIfNeeded(browserView)
            return
        }
        guard contentProcessTerminateRetryCount < maxContentProcessTerminateRetries else {
            triggerFallbackIfNeeded(reason: "content-process-terminated", for: browserView)
            return
        }
    ...
    func webView(_ browserView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if let requestURL = navigationAction.request.url,
           let rewrittenURL = telegramURL(from: requestURL) {
            if let currentURL = browserView.url,
            ...
            if browserView === self.primaryView {
                ...
                browserView.load(URLRequest(url: rewrittenURL))
            }
            decisionHandler(.cancel)
            return
        }
    ...
    func webView(_ browserView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
        if let response = navigationResponse.response as? HTTPURLResponse {
            let status = response.statusCode
            if status >= 400, navigationResponse.isForMainFrame {
                if shouldFallbackForHTTPStatus(status) {
                    triggerFallbackIfNeeded(reason: "http-\(status)", for: browserView)
                } else {
                }
            } else if status >= 400, browserView === popupView {
                closePopupIfNeeded(browserView)
            }
        }
        decisionHandler(.allow)
    ...
    func webView(_ browserView: WKWebView, createWebViewWith configuration: WKWebViewConfiguration, for navigationAction: WKNavigationAction, windowFeatures: WKWindowFeatures) -> WKWebView? {
        ...
        createdPopup.customUserAgent = browserView.customUserAgent
        createdPopup.translatesAutoresizingMaskIntoConstraints = false

        popupView?.removeFromSuperview()
        popupView = createdPopup
    ...
    func webViewDidClose(_ browserView: WKWebView) {
        browserView.removeFromSuperview()
        if popupView === browserView {
            popupView = nil
        }
    }

    private func handleNavigationError(_ error: Error, for browserView: WKWebView, stage: String) {
        let nsError = error as NSError
        if shouldIgnoreTransientNavigationError(nsError) {
            return
        }
        if handleUnsupportedURLIfNeeded(nsError, browserView: browserView, stage: stage) {
            return
        }
        if browserView === self.primaryView, retryMainLoadIfNeeded(after: nsError, stage: stage) {
            return
        }
        triggerFallbackIfNeeded(reason: "\(stage)-\(nsError.domain)-\(nsError.code)", for: browserView)
    }

    private func handleUnsupportedURLIfNeeded(_ error: NSError, browserView: WKWebView, stage: String) -> Bool {
        guard error.domain == NSURLErrorDomain, error.code == NSURLErrorUnsupportedURL else {
            return false
        }

        if let failedURL = failedNavigationURL(from: error, browserView: browserView),
           let rewrittenURL = telegramURL(from: failedURL) {
            if let currentURL = browserView.url,
               normalizedURLKey(currentURL) == normalizedURLKey(rewrittenURL) {
                return true
            }
    ...
    private func shouldIgnoreTransientNavigationError(_ error: NSError) -> Bool {
    ...
    private func telegramURL(from url: URL) -> URL? {
    ...
    private func failedNavigationURL(from error: NSError, browserView: WKWebView) -> URL? {
    ...
    private func closePopupIfNeeded(_ browserView: WKWebView) {
        guard browserView === popupView else { return }
        browserView.removeFromSuperview()
        popupView = nil
    }

    private func triggerFallbackIfNeeded(reason: String, for browserView: WKWebView?) {
        if let browserView, browserView !== self.primaryView {
            closePopupIfNeeded(browserView)
            return
        }
        ...
        NotificationCenter.default.post(
            name: .remoteFlowShouldShowFallback,
            object: nil,
            userInfo: [FlowFallback.reasonKey: reason]
        )
    }
